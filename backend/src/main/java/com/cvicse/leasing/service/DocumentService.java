...
//        document.setOrganizationId(String.valueOf(user.getOrganizationId()));
    ...
    public String update() {
        Query query = new Query(Criteria.where("sdk_version").is("v4.0"));

        Update update = new Update();
        update.set("interfaces.$[i].methods.$[j].method_name", "1111111222222233333");
        update.filterArray("i.interface_name", "QuoteDetail");
        update.filterArray("j.method_name", "QuoteDetailTest");
        if (documentRepository != null)
            documentRepository.updateFirst(query, update, "SDK_INFO");
        return "";
    }
}
